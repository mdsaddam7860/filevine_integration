...
const SYNC_STATE_FILE = path.join(process.cwd(), "hubspot-last-sync.json");
function getLastSyncDate() {
  try {
    if (!fs.existsSync(SYNC_STATE_FILE)) {
      // First run fallback: 24 hours back
    ...
    console.error("Failed to read lastSyncDate:", err.message);
    // Fail-safe fallback
    return new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
  }
}

function updateLastSyncDate(date = new Date()) {
  try {
    fs.writeFileSync(
      SYNC_STATE_FILE,
      JSON.stringify({ lastSyncDate: date.toISOString() }, null, 2),
      "utf8"
    );
  } catch (err) {
    console.error("Failed to update lastSyncDate:", err.message);
  }
}
...
function contactProperties() {
  return [
    ...
    "client_vehicle_make",
    "client_vehicle__model",
    "client_vehicle__year",
    "client_vehicle_license_plate_number",
    "client_vehicle_repair_estimate",
    "defendant_driver",
...
function dealProperties() {
  return [
    ...
    "defendants_names_list_all",
    // "body_shop",
    "client_vehicle__make",
    "passenger_contact_phone_number",
    "hubspot_owner_id",
    "registered_vehicle_owner",
    "defendant_2_vehicle_registered_owner",
    "attorneys_name",
    "workers_compensation_attorney",
    "spouse_name",
    "body_shop_type",
    "ambulance_type",
    "pd_letter_sent",
